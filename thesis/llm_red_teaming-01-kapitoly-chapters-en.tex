...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=0.7\linewidth,height=15cm]{obrazky-figures/multi-turn.pdf}\\[1pt]
  \caption{Illustration of a multi-turn jailbreak attack, in which an attacker iteratively refines prompts based on previous model responses to progressively elicit disallowed behavior. Adapted from Zhao and Zhang~\cite{zhao2025siren}.}
...
\begin{figure}[h]\centering
  ...
  \caption{Illustration of a universal trigger attack, where a fixed trigger sequence appended to user inputs induces undesired behavior in an unmodified language model across different prompts and, in some cases, across different models.}
...
An example is the black-box Trojan prompt attack framework \emph{TrojLLM}, which demonstrates that stealthy trigger patterns can be embedded into widely used LLM architectures and APIs, enabling malicious behavior when the trigger is present~\cite{xue2023trojllm}.
Because the trigger is designed to appear benign, such attacks may bypass input sanitization and heuristic defenses.
