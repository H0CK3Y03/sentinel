...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=0.7\linewidth,height=4cm]{obrazky-figures/single-turn.pdf}\\[1pt]
...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=\linewidth,height=3cm]{obrazky-figures/prompt-injection.pdf}\\[1pt]
...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=0.7\linewidth,height=14cm]{obrazky-figures/multi-turn.pdf}\\[1pt]
...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=\linewidth,height=10cm]{obrazky-figures/universal-triggers.pdf}\\[1pt]
...
A related but distinct class of attacks are Trojan-style (backdoor) attacks, where the model itself is modified during training or fine-tuning to respond maliciously to a specific trigger.
An example is the black-box Trojan prompt attack framework \emph{TrojLLM}, which demonstrates that stealthy trigger patterns can be embedded into widely used LLM architectures and APIs, enabling malicious behaviour when the trigger is present~\cite{xue2023trojllm}.
Because the trigger is designed to appear benign, such attacks may bypass input sanitisation and heuristic defences.

\begin{figure}[h]\centering
  \centering
  \includegraphics[width=0.5\linewidth,height=7cm]{obrazky-figures/trojan-style.pdf}\\[1pt]
  \caption{
    Illustration of a Trojan-style (backdoor) attack on a large language model. The modified model behaves normally for benign inputs but produces malicious or policy-violating outputs when a specific trigger pattern is present.
...
\begin{figure}[h]\centering
  \centering
  \includegraphics[width=0.5\linewidth,height=6.7cm]{obrazky-figures/evasion-perturbation.pdf}\\[1pt]
...
We first outline the design requirements derived from the analysis in the preceding chapters.
Subsequently, we present the overall system architecture.
The chapter then describes the structure of the toolkit and the interactions between its main components.
...
This section summarises the functional and non-functional requirements that guided the design of the toolkit and ties them to gaps identified in Chapter~\ref{ch:red-teaming} section~\ref{sec:gaps}.
...
\begin{itemize}
  ...
  \item \textbf{Reproducibility:} deterministic experiment manifests (seed, model config, attack generator config, prompts) and machine-readable logs to allow reruns and regression testing.
...
\subsection{Mapping to gaps from Chapter~\ref{ch:red-teaming} section~\ref{sec:gaps}}
...
(4) support diverse backends via model adapters
and (5) increase reproducibility via manifests and JSONL logging.

\section{High-level Architecture}

Figure~\ref{design-figure} presents the component-level architecture.
The following paragraphs describe components and the main data/control flows.

\subsection{Components (narrative)}

\begin{itemize}
  \item \textbf{Experiment manifest:} a declarative, versioned specification describing the experiment (target model(s), attack generator selection, judge configuration, seeds, limits).
  \item \textbf{Orchestrator:} the central coordinator that loads manifests, schedules attack generation, sends prompts to model adapters, invokes judges, and records structured logs.
  \item \textbf{Attack generators:} plugins that produce candidate prompts (manual lists, templates, or automated generators such as TAP-style searchers or LLM-as-attacker modules). Generators implement a small interface (request/produce).
  \item \textbf{Model adapters:} adapters abstracting backend-specific API calls and rate-limits; they translate orchestrator requests into backend-specific invocations and return raw responses.
  \item \textbf{Judge adapters:} evaluation modules that classify outputs (heuristic detectors, ML classifiers, or LLM-based judges). Judges write structured verdicts to the orchestrator and log store.
  \item \textbf{Log store (JSONL):} append-only records capturing prompt, model response, verdict(s), and experiment metadata. Serves as single source of truth for reporting and reproducibility.
  \item \textbf{Reporting UI:} a read-only interface that indexes or streams JSONL logs to present aggregated metrics, failed cases, and raw dialogues.
\end{itemize}

\subsection{Principal data and control flows}

Refer to Figure~\ref{design-figure} for the visual flow.
Key flows:
\begin{enumerate}
  \item \textbf{Manifest $\rightarrow$ Orchestrator:} orchestrator loads the manifest describing experiment parameters.
  \item \textbf{Orchestrator $\rightarrow$ Attack generators:} the orchestrator initialises and invokes the selected attack generator according to the manifest and requests candidate prompts during experiment execution.
  ...
  \item \textbf{Judge adapters $\rightarrow$ Log store:} judges append structured verdicts and labels.
  \item \textbf{Orchestrator $\rightarrow$ Log store:} stores experiment metadata, prompts, raw responses and orchestration events.
  \item \textbf{Log store $\rightarrow$ Reporting UI:} the UI reads logs to visualise and export results.
\end{enumerate}

\section{Module Specifications}

This section presents concrete interfaces, expected behaviour, and configuration knobs for each core module.

\subsection{Model Adapters}

\paragraph{Purpose.} Provide a uniform interface to execute prompts on heterogeneous backends.
...
\begin{itemize}
  \item \textbf{Local LLaMA/llama.cpp} — lightweight, free option for both generation and judging.
...
\begin{itemize}
  \item translate generic call into backend-specific payload,
  \item manage rate limits, errors, retries and backoff policies,
  \item normalise response object (text, tokens, metadata),
...
\paragraph{Purpose.} Produce adversarial prompts or prompt sequences under an agreed interface.
...
\paragraph{Types}
\begin{itemize}
  \item \textbf{Manual/template:} curated lists or parametric templates (simple and reproducible).
  \item \textbf{Search-based (TAP-like):} tree/beam search over prompt space (query-efficient).
  \item \textbf{LLM-driven:} use an LLM attacker to synthesise candidate prompts (requires judge validation).
\end{itemize}

\paragraph{Notes}
Generators should be stateless between experiments unless the manifest explicitly requests stateful policies (e.g., multi-turn attackers).

\subsection{Judges and Evaluation}

\paragraph{Purpose.} Decide whether a given model output constitutes a success for a given attack objective and produce structured metadata.
...
\paragraph{Logging format (JSONL)}  
...
\paragraph{Log store properties}
\begin{itemize}
  \item append-only and immutable (to aid reproducibility and auditing)
  \item compact and machine-readable (JSONL encourages streaming analysis)
  \item optionally indexed for fast UI queries (SQLite / small local index)
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{PII handling:} redact or avoid storing sensitive user data; if necessary, store only hashed identifiers and be explicit about retention policies.
...
\paragraph{Technology choices (conceptual)}
\begin{itemize}
  \item \textbf{Language:} Python (ecosystem, packaging, plugin support).
  \item \textbf{Model runtime:} llama.cpp as the core runtime with a Python binding (e.g., \texttt{llama-cpp-python}) used in the adapter. Use quantised model files (GGUF) for low-memory operation where appropriate.
  \item \textbf{Orchestrator:} simple Python service (CLI using Typer, async runner using asyncio).
  \item \textbf{Log store:} append-only JSONL files on disk as the canonical experiment log.
  \item \textbf{UI:} lightweight read-only web app (Streamlit or Flask + small frontend) that reads logs/index.
  \item \textbf{Deployment:} Docker + docker-compose for reproducible local deployments.
\end{itemize}
...
Newline-delimited JSON (JSONL) is used as the canonical append-only logging format for experiment results.
For improved query performance in the user interface, an optional lightweight index (e.g., SQLite) may be derived from the JSONL logs.
...
\begin{itemize}
  \item \textbf{Resource constraints:} the design assumes documentation of expected RAM and CPU requirements for representative model sizes, with recommendations for smaller models during laptop-based development.
  \item \textbf{Security and containment:} untrusted plugins may be executed in isolated subprocesses or lightweight containers; model outputs are never automatically executed.
  \item \textbf{Observability:} the orchestrator may expose basic health checks and runtime metrics (e.g., request rate, latency, queue length, judge throughput) to support debugging and evaluation.
\end{itemize}
