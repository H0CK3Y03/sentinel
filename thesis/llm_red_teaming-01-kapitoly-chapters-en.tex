...
\section{Parallel Batch Execution}\label{sec:parallel-batch-execution}

The implementation follows the design goal of bounded parallel execution by processing each batch of prompts concurrently.
For every batch, the orchestrator creates a limited number of asynchronous prompt tasks, each of which performs model inference and local judging independently of the other prompts in the same batch.
This keeps the implementation compact while allowing the toolkit to use available backend capacity more effectively than a purely sequential loop.

Parallel execution is intentionally constrained by a manifest parameter so that the user can trade throughput against resource consumption.
When the configured concurrency is set to one, the runtime behaves exactly like the sequential implementation.
When the limit is increased, the orchestrator keeps the logical order of the experiment intact by recording results after all prompt tasks in the batch have completed, then updating adaptive generators and appending the JSONL records in the original prompt order.

This design is particularly useful for local testing and for backends that can serve multiple requests at once, while still remaining predictable enough for reproducible red-teaming experiments.
