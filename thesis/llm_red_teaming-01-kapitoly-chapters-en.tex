...
Adapter--generator pairs are independent units of work, so the orchestrator runs them through an outer \texttt{asyncio.Semaphore} bounded by the manifest parameter \texttt{max\_combo\_concurrency}.
Multi-adapter experiments benefit most from this level: testing several models with the same generators in parallel cuts time roughly linearly in the number of combos.
...
\begin{figure}[htbp]
  ...
  \caption{Four nested levels of parallelism. The outer experiment holds an \texttt{asyncio.Semaphore} that bounds the number of concurrent adapter-generator combos.
  Within each combo, a second semaphore caps concurrent prompts, for each prompt, judges fan out via \texttt{asyncio.gather} and an optional streaming task prefetches the next batch while the current one is still being judged.}
...
The orchestrator's solution is to advance an open conversation within the trial that started it, rather than waiting for the next outer batch to surface the next turn.
...
\begin{figure}[htbp]
  ...
  \caption{Multi-turn conversation timeline. The generator produces an initial prompt, the orchestrator drives it through the adapter and judges, then calls \texttt{update()} on the generator and asks for the next turn. The loop continues until one of three exit conditions fires.
  Each exit writes a structured \texttt{stop\_reason} into the JSONL log.}
...
The analyser deliberately reads each trial's pre-aggregated \texttt{final\_verdict} field rather than recomputing the ensemble from the per-judge verdicts.
This keeps the offline report consistent with the live experiment summary, which is the more authoritative view because it is what the orchestrator emitted.
...
During a \texttt{run}, the orchestrator's four callback hooks (\texttt{on\_generation\_start}, \texttt{on\_adapter\_start}, \texttt{on\_judge\_start}, and \texttt{on\_trial\_complete}) are wired into a small in-place rendering panel that uses ANSI cursor-up escapes to overwrite the previous frame.
...
Concurrency tests verify that the rate limiter respects the requested budget under contention.
Manifest tests check that YAML and JSON inputs round-trip through the loader and that auto-generated instance IDs are stable across re-loads.
Judge tests ensure the heuristic judge picks the right label on canonical refusal, compliance, and neutral responses.
Orchestrator tests run the full pipeline end-to-end against the stub adapter and verify both the JSONL log shape and the parallelism behaviour (combo-, prompt-, judge-, and streaming-level).
Multi-turn tests check that conversations advance sequentially within their combo, that isolation flags create the expected number of generator instances, and that the \texttt{stop\_reason} field is emitted on every exit path.
And the OpenAI generator and judge tests use \texttt{unittest.mock} to substitute the HTTP layer and exercise the configuration, generation, and verdict-parsing logic without making any real network calls.
