...
% This chapter contains many \texttt{}-set identifiers that LaTeX cannot
% hyphenate; \sloppy widens the spacing tolerance to keep the right margin
% straight without forcing manual line breaks.
\sloppy
...
The earlier chapter on design fixed the architecture; here we describe how that architecture is realised as a concrete Python package, how the modules are wired together, and how the runtime behaves under load.
...
Subsequent sections drill into the data model, the plugin system, the three component families (adapters, generators, judges), the orchestrator that drives them, the layered parallelism strategy, the multi-turn extension, and the logging and analysis pipeline.
...
Most of the toolkit is written using only the standard library; the small set of third-party dependencies is intentionally limited to \texttt{typer} for the CLI, \texttt{pyyaml} for the optional YAML manifest loader, and \texttt{httpx} as a single HTTP client used by every OpenAI-compatible component.
...
Every component in the toolkit speaks the same handful of types, and almost all of them live in \texttt{models.py}.
...
It carries a list of labels (drawn from \texttt{refusal}, \texttt{compliance}, and \texttt{inconclusive}), a numeric \texttt{confidence}, a \texttt{judge\_weight} used during ensemble aggregation, the judge type (one of \texttt{HEURISTIC}, \texttt{CLASSIFIER}, \texttt{LLM}, or \texttt{ENSEMBLE}), and a free-text \texttt{explanation} that downstream tools can show to users.
...
Component construction happens through three small factories, \texttt{create\_adapter}, \texttt{create\_generator}, and \texttt{create\_judge}, that look the class up by name, instantiate it, and stamp it with the requested \texttt{instance\_id}.
...
The adapter also supports two more advanced white-box capabilities.
...
The multi-turn generator draws from six pre-authored conversational flows --- \emph{gradual\_escalation}, \emph{hypothetical\_framing}, \emph{context\_switching}, \emph{roleplay\_progression}, \emph{normalisation}, and \emph{refusal\_erosion} --- each defined as an ordered list of prompts that gradually steer the model toward harmful output.
...
They also expose \texttt{get\_next\_turn(conversation\_id)}, which the orchestrator polls to advance an open conversation through additional turns within the same outer batch.
...
The lifecycle helpers in \texttt{orchestrator\_lifecycle.py} apply the manifest configuration to every unique component, merging the global seed into each generator's configuration so that the experiment is reproducible end-to-end.
