...
The system is organised around a small set of well-defined components, each with a clearly separated responsibility.
Experiments are specified using declarative manifests, which describe the target models, attack generators, judging configuration, and execution parameters.
These manifests are consumed by the orchestrator, which serves as the central coordination component of the toolkit.

During execution, the orchestrator invokes attack generators to obtain candidate adversarial prompts and forwards these prompts to model adapters.
Model adapters abstract away backend-specific details and provide a uniform interface for interacting with local or API-based language models.
Model outputs are then passed to the judging pipeline, which evaluates responses using a combination of heuristic checks, classifier-based methods, and optional LLM-based judges.

Both orchestration events and judging results are recorded in an append-only JSONL log store, which acts as the single source of truth for experiment outcomes.
A lightweight reporting interface consumes these logs to provide aggregated metrics, inspect individual failures, and support further analysis.
